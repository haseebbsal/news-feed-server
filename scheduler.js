...
function begin() {
    ...
    cron.schedule('* * * * *', async () => {
        ...
        for (let row of getAllScheduled) {
            ...
            if (
                (currentDate === nextDate) &&
                 moment().hour() === periodicity.hour &&
                 moment().minute() === periodicity.minute
               ) {
                ...
                for (let p of totalArticleUrls) {
                    ...
                    if (!checkIfAlreadyPublishedUrl && !lowRelevanceArticles.includes(p)) {
                        ...
                        if (limitCheck !== limit) {
                            const {
                                message,
                                relevanceIndex: relevanceIndexx,
                                original,
                                summary,
                                rewritten,
                                title,
                                ...
                                rewriteImage,
                                files
                            } = await new Promise(async (resolvee, reject) => {
                            ...
                            if (!message) {
                                ...
                                if (original) {
                                    ...
                                    const publishArticle = await publishedArticleModel.create({
                                        userId,
                                        article: original,
                                        title,
                                        articleUrl: link,
